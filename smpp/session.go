...
func CreateTableAndInsertValues(src, dst, msg string) error {
	...
	_, err = db.Exec("INSERT INTO messages (SourceAddress, DestinationAddress, Message, Reception) VALUES (?, ?, ?, NOW())", src, dst, msg)
	if err != nil {
		return err
	}
...
func getPercentBySmppAddress(smppAddress string) (string, error) {
	db, err := sql.Open("mysql", "root:@tcp(localhost:3306)/templates")
	if err != nil {
		return "", err
	}
	...
	var percent string
	query := "SELECT Percent FROM templates.generator_REC WHERE SmppAddress = ?"
	err = db.QueryRow(query, smppAddress).Scan(&percent)
	if err != nil {
		return "", err
	}
	return percent, nil
...
func (s *Session) Listen(ip string) error {
	fmt.Println("Start Listning for .........", ip, "\n\n\n\n\n")
	i := 0
	per, _ := getPercentBySmppAddress(ip)
	percent, _ := strconv.Atoi(per)
	for {
		p, err := s.c.Decode()
		if err != nil {
		...
		case pdu.SubmitSMID:
			...
			if i == 100 {
				...
				s.log.Infof("\n\n We sends dlrs Source : (%s) , Dst: (%s) , Msg: (%s) \n\n", pdufield.SourceAddr, pdufield.DestinationAddr)
				fmt.Println("\n\n\n\n Test!!!!!!!!!!\n\n\n\n\n\n\n")
...
func (s *Session) DLRs(p pdu.Body) {
	go func() {
		...
		case 1:
			...
			s.log.Infof("\nDLR DELIVERED (%d)", dlr.Header().Seq)
		case 2:
...
func createDLR(p pdu.Body, state int) pdu.Body {
	...
	f := dlr.Fields()
	var src_ip pdufield.Body = src["source_addr"]
	srcstr := src_ip.String()
	var dst_ip pdufield.Body = src["destination_addr"]
	dststr := dst_ip.String()
	var msg_ip pdufield.Body = src["short_message"]
	msgstr := msg_ip.String()
	fmt.Println("\n\n\n\n\n\n\nWe Are in Create Dlr Func ........ Src:", srcstr, ", Dst:", src["destination_addr"], "Msg:", src["short_message"], "Pdufield:", f, "\n\n\n\n\n\n\n\n")
	CreateTableAndInsertValues(srcstr, dststr, msgstr)
